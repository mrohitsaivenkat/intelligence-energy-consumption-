...
export const INDIAN_DISCOMS = [
  { name: 'MSEDCL (Mahavitaran - Maharashtra)', rate: 8.5, fixed: 280, tax: 350 },
  { name: 'Tata Power / Adani Electricity (Mumbai MMR)', rate: 8.2, fixed: 300, tax: 360 },
  { name: 'BESCOM (Bengaluru & South Karnataka)', rate: 8.0, fixed: 220, tax: 310 },
  { name: 'BSES Rajdhani / Yamuna (Delhi NCR)', rate: 7.0, fixed: 250, tax: 290 },
  { name: 'Tata Power DDL (North / NW Delhi)', rate: 7.0, fixed: 250, tax: 290 },
  { name: 'TANGEDCO (Tamil Nadu & Chennai)', rate: 7.2, fixed: 180, tax: 280 },
  { name: 'TSSPDCL / TSNPDCL (Hyderabad & Telangana)', rate: 7.5, fixed: 200, tax: 300 },
  { name: 'APEPDCL / APCPDCL (Andhra Pradesh)', rate: 7.3, fixed: 190, tax: 280 },
  { name: 'WBSEDCL / CESC (Kolkata & West Bengal)', rate: 7.6, fixed: 240, tax: 320 },
  { name: 'Torrent Power / GUVNL (Gujarat)', rate: 6.8, fixed: 210, tax: 270 },
  { name: 'JVVNL / AVVNL / JdVVNL (Rajasthan)', rate: 7.8, fixed: 260, tax: 330 },
  { name: 'UPPCL - PVVNL / MVVNL / DVVNL (Uttar Pradesh)', rate: 7.5, fixed: 240, tax: 310 },
  { name: 'PSPCL (Punjab State Power Corp)', rate: 7.1, fixed: 200, tax: 290 },
  { name: 'DHBVN / UHBVN (Haryana - Gurugram / Faridabad)', rate: 6.9, fixed: 220, tax: 280 },
  { name: 'KSEB (Kerala State Electricity Board)', rate: 7.2, fixed: 180, tax: 270 },
  { name: 'TPCODL / TPNODL / TPSODL (Odisha)', rate: 6.5, fixed: 170, tax: 250 },
  { name: 'NBPDCL / SBPDCL (Bihar State Power)', rate: 7.1, fixed: 190, tax: 280 },
  { name: 'CSPDCL (Chhattisgarh)', rate: 6.6, fixed: 180, tax: 260 },
  { name: 'MPPKVVCL / MPMKVVCL (Madhya Pradesh)', rate: 7.4, fixed: 230, tax: 300 },
  { name: 'APDCL (Assam & North East)', rate: 7.4, fixed: 190, tax: 270 },
  { name: 'Other State Electricity Board / DISCOM', rate: 7.2, fixed: 200, tax: 280 }
];
...
const BillAnalyzer = () => {
  ...
  const [formData, setFormData] = useState({
    billingPeriod: uploadedBill?.billingPeriod || 'August 2026',
    unitsConsumed: uploadedBill?.units || 412,
    tariffRate: uploadedBill?.tariffRate || 7.5,
    fixedCharges: uploadedBill?.fixedCharges || 250,
    taxes: uploadedBill?.taxes || 318,
    otherCharges: uploadedBill?.otherCharges || 150,
    discom: uploadedBill?.discom || 'MSEDCL (Mahavitaran - Maharashtra)'
  });
          ...
          <form onSubmit={handleAnalyze} className="space-y-6">
            {mode === 'upload' ? (
              <div className="space-y-4">
                <div 
                  className="border-2 border-dashed border-white/15 rounded-3xl p-8 sm:p-10 text-center hover:border-cyan-500/50 transition-all cursor-pointer group bg-white/[0.02]"
                ...
                >
                  ...
                  <p className="text-xs text-slate-500">Supports MSEDCL, BESCOM, Tata Power, Adani, BSES, TANGEDCO & all state DISCOM bills (Max 10MB)</p>
                </div>

                {/* Quick 1-click Sample Bills for Testing Across India */}
                <div className="p-4 bg-white/5 border border-white/10 rounded-2xl">
                  <p className="text-xs font-semibold text-slate-300 mb-2">Or test with a representative Indian DISCOM sample bill:</p>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      type="button"
                      onClick={() => {
                        setFile({ name: 'MSEDCL_Mumbai_August2026.pdf' });
                        setFormData({
                      ...
                      className="text-left px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-xs text-slate-300 border border-white/5 transition-all"
                    >
                      <span className="font-bold text-white block">Maharashtra</span>
                      MSEDCL • 440 units (₹4,540)
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        setFile({ name: 'BESCOM_Bengaluru_August2026.pdf' });
                        setFormData({
                          billingPeriod: 'August 2026',
                          unitsConsumed: 380,
                          tariffRate: 8.0,
                          ...
                          taxes: 310,
                          otherCharges: 140,
                          discom: 'BESCOM (Bengaluru & South Karnataka)'
                        });
                      }}
                      className="text-left px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-xs text-slate-300 border border-white/5 transition-all"
                    >
                      <span className="font-bold text-white block">Karnataka</span>
                      BESCOM • 380 units (₹3,710)
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        setFile({ name: 'BSES_Delhi_August2026.pdf' });
                        setFormData({
                          billingPeriod: 'August 2026',
                          unitsConsumed: 410,
                          tariffRate: 7.0,
                          ...
                          taxes: 290,
                          otherCharges: 130,
                          discom: 'BSES Rajdhani / Yamuna (Delhi NCR)'
                        });
                      }}
                      className="text-left px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-xs text-slate-300 border border-white/5 transition-all"
                    >
                      <span className="font-bold text-white block">Delhi NCR</span>
                      BSES • 410 units (₹3,540)
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        setFile({ name: 'TANGEDCO_Chennai_August2026.pdf' });
                        setFormData({
                          billingPeriod: 'August 2026',
                          unitsConsumed: 460,
                          tariffRate: 7.2,
                          ...
                          taxes: 280,
                          otherCharges: 120,
                          discom: 'TANGEDCO (Tamil Nadu & Chennai)'
                        });
                      }}
                      className="text-left px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-xs text-slate-300 border border-white/5 transition-all"
                    >
                      <span className="font-bold text-white block">Tamil Nadu</span>
                      TANGEDCO • 460 units (₹3,892)
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1">Select Your Electricity DISCOM (Pan-India)</label>
                  <select
                    value={formData.discom}
                    onChange={(e) => {
                      const selected = INDIAN_DISCOMS.find(d => d.name === e.target.value);
                      if (selected) {
                        setFormData({
                          ...formData,
                          discom: selected.name,
                          tariffRate: selected.rate,
                          fixedCharges: selected.fixed,
                          taxes: selected.tax
                        });
                      } else {
                        setFormData({ ...formData, discom: e.target.value });
                      }
                    }}
