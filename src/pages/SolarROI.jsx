...
const STATE_CONFIG = {
  'Andhra Pradesh': { tariff: 7.3, sunHours: 5.5, discom: 'APEPDCL / APCPDCL' },
  'Assam': { tariff: 7.4, sunHours: 4.6, discom: 'APDCL' },
  'Bihar': { tariff: 7.1, sunHours: 5.0, discom: 'NBPDCL / SBPDCL' },
  'Chandigarh': { tariff: 5.8, sunHours: 5.2, discom: 'Chandigarh Electricity Dept' },
  'Chhattisgarh': { tariff: 6.6, sunHours: 5.4, discom: 'CSPDCL' },
  'Delhi': { tariff: 7.0, sunHours: 5.1, discom: 'BSES / TPDDL / NDMC' },
  'Goa': { tariff: 5.4, sunHours: 5.3, discom: 'Goa Electricity Dept' },
  'Gujarat': { tariff: 6.8, sunHours: 5.6, discom: 'Torrent / GUVNL / DGVCL' },
  'Haryana': { tariff: 6.9, sunHours: 5.2, discom: 'DHBVN / UHBVN' },
  'Himachal Pradesh': { tariff: 5.2, sunHours: 4.8, discom: 'HPSEBL' },
  'Jammu & Kashmir': { tariff: 4.8, sunHours: 4.9, discom: 'JPDCL / KPDCL' },
  'Jharkhand': { tariff: 6.4, sunHours: 5.2, discom: 'JBVNL' },
  'Karnataka': { tariff: 8.0, sunHours: 5.3, discom: 'BESCOM / HESCOM / MESCOM' },
  'Kerala': { tariff: 7.2, sunHours: 5.1, discom: 'KSEB' },
  'Madhya Pradesh': { tariff: 7.4, sunHours: 5.5, discom: 'MPPKVVCL / MPMKVVCL' },
  'Maharashtra': { tariff: 8.5, sunHours: 5.2, discom: 'MSEDCL / Tata Power / Adani' },
  'Odisha': { tariff: 6.5, sunHours: 5.2, discom: 'TPCODL / TPNODL / TPSODL' },
  'Punjab': { tariff: 7.1, sunHours: 5.2, discom: 'PSPCL' },
  'Rajasthan': { tariff: 7.8, sunHours: 5.8, discom: 'JVVNL / AVVNL / JdVVNL' },
  'Tamil Nadu': { tariff: 7.2, sunHours: 5.4, discom: 'TANGEDCO' },
  'Telangana': { tariff: 7.5, sunHours: 5.4, discom: 'TSSPDCL / TSNPDCL' },
  'Uttar Pradesh': { tariff: 7.5, sunHours: 5.2, discom: 'UPPCL (PVVNL / MVVNL)' },
  'Uttarakhand': { tariff: 5.6, sunHours: 5.0, discom: 'UPCL' },
  'West Bengal': { tariff: 7.6, sunHours: 4.8, discom: 'WBSEDCL / CESC Kolkata' },
  'Other States & UTs': { tariff: 7.2, sunHours: 5.2, discom: 'State Electricity Board' },
};
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white/5 border border-white/10 p-8 rounded-3xl backdrop-blur-sm h-fit space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-1">State / DISCOM Region (Pan-India)</label>
              <select
              ...
              >
                {Object.entries(STATE_CONFIG).map(([stateName, cfg]) => (
                  <option key={stateName} value={stateName} className="bg-slate-900 text-white">
                    {stateName} • {cfg.discom} (₹{cfg.tariff}/kWh, {cfg.sunHours} hrs sun)
                  </option>
                ))}
              </select>
            </div>
